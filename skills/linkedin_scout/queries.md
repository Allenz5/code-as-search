...
**类型比 query 值钱得多，而且到目前为止全是人想出来的，不是机制产出的。** 招聘帖和
线下活动这两个最有价值的类型都是 Allen 提的。agent 侧唯一的机制是 SKILL.md 里那条
反向查找（读已 👍 的人自己发什么帖），它是个弱生成器。**所以每轮报告都要主动问一句
有没有新类型** —— 这不是客套，是这个系统目前最缺的输入。
